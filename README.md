...
| `NEXT_PUBLIC_TURNSTILE_SITE_KEY` | Public | Cloudflare Turnstile site key (recommended in production) |
| `TURNSTILE_SECRET_KEY` | Server | Cloudflare Turnstile secret (recommended in production) |

Contact submissions need Supabase. When **both** Turnstile keys are set, captcha is enforced server-side **before** any Supabase write or Resend send. Without Turnstile, honeypot + rate limits still apply. `SITE_URL` may be apex or `www` — both origins are accepted. Local/CI can use Cloudflare always-pass dummy keys (`1x00000000000000000000AA` / `1x0000000000000000000000000000000AA`).
